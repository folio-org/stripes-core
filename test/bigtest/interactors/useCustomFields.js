...
@interactor
class UseCustomFieldsInteractor {
...
const UseCustomFieldsInteractor = HTML.extend('UseCustomField')
  ...
  .actions({
    fillUserName: ({ find }, value) => {
      find(TextField({ id: 'input-username' }).fill(value));
    },
    fillPassword: ({ find }, value) => {
      find(TextField({ id: 'input-password' }).fill(value));
    },
    clickLogin: ({ find }) => {
      find(Button({ id: 'clickable-login' })).click();
    },
    clickProfileDropdown: ({ find }) => {
      find(Button({ id: 'profileDropdown' })).click();
    },
    clickLogout: ({ find }) => {
      find(Button({ id: 'clickable-logout' })).click();
    },
  });
