...
* Conditionally use `/users-keycloak/_self` endpoint when `users-keycloak` interface is present and `bl-users` if not in useUserSelfTenantPermissions. Refs STCOR-905.
