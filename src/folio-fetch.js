 ...
 * @throws if RTR fails
 */
export const rtr = async (event, handler) => {
  ...
  isRotating = true;
  return handler(`${okapiUrl}/authn/refresh`, {
    headers: {
 ...
 * @returns Promise
 */
const passThroughWithRT = (event, handler) => {
  return rtr(event, handler)
    .then(() => {
        ...
        return response.clone().text()
          .then(text => {
            ...
            if (text.startsWith('Token missing')) {
              ...
              return passThroughWithRT(event, handler);
            }
 ...
 * @throws if any fetch fails
 */
export const passThrough = (event, te, oUrl, handler) => {
  const req = event.request.clone();
  ...
  if (isOkapiRequest(req, oUrl)) {
    ...
    if (isLogoutRequest(req, oUrl)) {
      return passThroughLogout(event, handler);
    }

    if (isPermissibleRequest(req, te, oUrl)) {
      return passThroughWithAT(event, handler);
    }
...
/** HOPEFULLY WE CAN HANDLE THIS MORE DIRECTLY WITH HANDLERS IN STRIPES */
/**
...
class FolioFetch {
  constructor() {
    ...
    global.fetch = this.fetchInterceptor.bind(this);
    console.log('folio-fetch in place. Fetch and XMLHttpReqests overridden');
    this.tokenExpiration = null;
    this.init();
  }

  async init() {
    const { tokenExpiration: te } = await localforage.getItem('okapiSess');
    this.tokenExpiration = te;
  }

  fetchInterceptor(url, options) {
    let request = url;
    if (typeof request !== 'symbol') request = new Request(url, options);
