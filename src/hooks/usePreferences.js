...
export default () => {
  ...
  const getPreference = useCallback(
    async ({ scope, key, userId: providedUserId }) => {
      // eslint-disable-line
      ...
      try {
        resp = await ky.get(
          `settings/entries?query=userId=="${_userId}" and scope=="${scope}" and key=="${key}"`,
        );
        if (resp.ok) {
    ...
    [ky, userId, setId], // eslint-disable-line react-hooks/exhaustive-deps
  );

  const setPreference = useCallback(
    async ({ scope, key, value, userId: providedUserId }) => {
      const _userId = providedUserId || userId;
    ...
    [idRef, ky, userId, setId], // eslint-disable-line react-hooks/exhaustive-deps
  );
