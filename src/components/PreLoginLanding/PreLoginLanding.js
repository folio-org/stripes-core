...
function PreLoginLanding({ onSelectTenant }) {
  ...
  const getLoginUrl = () => {
    ...
    if (okapi.authnUrl) {
      return `${okapi.authnUrl}/realms/${okapi.tenant}/protocol/openid-connect/auth?client_id=${okapi.clientId}&response_type=code&redirect_uri=${redirectUri}&unauthorized_redirect=${window.location.pathname}&scope=openid&isConsortium=true`;
    }
    return '';
  };
