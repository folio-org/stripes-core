...
const WarningBanner = ({ interfaces, modules, bannerRef }) => {
  ...
  const incompatibleModules = useDeepCompareMemo(
    () =>
      Object.entries(allInterfaces)
        .filter(
          ([key]) => key in interfaces && !isVersionCompatible(interfaces[key], allInterfaces[key]),
        )
        .map(([key, value]) => `${key} ${value}`),
    [allInterfaces, interfaces],
  );
  ...
  const missingModulesMsg = (
    <FormattedMessage
      id="stripes-core.about.missingModuleCount"
      values={{ count: missingModulesCount }}
    />
  );
  const incompatibleModuleMsg = (
    <FormattedMessage
      id="stripes-core.about.incompatibleModuleCount"
      values={{ count: incompatibleModulesCount }}
    />
  );

  return (
    <div className={css.warningContainer}>
      <MessageBanner type="warning" show={!!missingModulesCount} dismissable ref={bannerRef}>
        <Headline>{missingModulesMsg}</Headline>
        <List listStyle="bullets" items={missingModules} />
      </MessageBanner>

      <MessageBanner type="warning" show={!!incompatibleModulesCount} dismissable ref={bannerRef}>
        <Headline>{incompatibleModuleMsg}</Headline>
        <List listStyle="bullets" items={incompatibleModules} />
      </MessageBanner>
    </div>
  );
};
