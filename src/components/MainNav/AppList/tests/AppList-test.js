...
describe('AppList', () => {
  ...
  describe('opening the appList with a selected app', () => {
    beforeEach(async () => {
      await mountWithContext(
        ...
        <div style={{ width: 150, background: 'yellow' }}>
          <BrowserRouter>
            <AppList apps={apps} selectedApp={selectedApp} dropdownToggleId="xyz" />
          </BrowserRouter>
        </div>,
      );
    ...
    it('focuses the corresponding item for the selected app', () =>
      AppMenuInteractor({ focusedIndex: -1 }).absent());
  ...
  describe('If there is no apps to show', () => {
    beforeEach(async () => {
      await mountWithContext(
        <BrowserRouter>
          <AppList apps={[]} selectedApp={{}} dropdownToggleId="xyz" />
        </BrowserRouter>,
      );
    });
  ...
  describe('If the screen width is small and there are hidden apps', () => {
    beforeEach(async () => {
      await mountWithContext(
        ...
        <div style={{ width: 150, background: 'yellow' }}>
          <BrowserRouter>
            <AppList apps={apps} dropdownToggleId="xyz" />
          </BrowserRouter>
        </div>,
      );
      await appsNav.focusDropdownTrigger();
    ...
    it('Should focus the first item in the dropdown if there is no current app', () =>
      appsDropdown.has({ focusedIndex: 0 }));
    ...
    describe('Clicking an item inside the app list dropdown', () => {
      ...
      it('Should close the app dropdown and focus the dropdown toggle', () =>
        Bigtest.Button('Apps').is({ focused: true }));
    });
  });
});
